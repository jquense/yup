...
Yup é um "JavaScript schema builder"(Construtor de esquemas javascript) para parseamento e validação de valores. Define um schema, transforma um valor para combinar, valida a forma de um valor(objetos geralmente) existente ou ambos. Schemas do yup são extremamente expressivos e permitem modelamentos complexos, validações interdependentes ou transformação de valor.

A api do Yup é fortemente inspirada pelo [Joi](https://github.com/hapijs/joi), só que mais enxuto e feito para validações no lado do cliente como caso primário de uso. Yup separa as funções de parseamento e validações em etapas separadas. `cast()` transforma dados enquanto `validate` checa se a entrada tem o formato correto. Cada um pode performar junto (como a validação de formulários HTML) ou separadamente (como a desserialização de dados confiáveis de APIs)

**Experimente:** https://runkit.com/jquense/yup#
...
- [Uso](#uso)
  - [Usando um dicionário de locale personalizado](#usando-um-dicionário-de-locale-personalizado)
- [API](#api)
  - [`yup`](#yup)
...
npm install -S yup
```
Yup sempre depende do objeto global `Promise` para manipular valores de forma assíncrona como `Set` e `Map`. Para browsers que não suportam isso, você vai precisar incluir um polyfill, como core-js:
...
Se você está usando TypeScript, instalar os tipos do Yup é recomendado
...
Você define e cria objetos schema. Objetos schema são imutáveis, então cada chamada de método return um _novo_ objeto schema. Quando a sinxate esmodule está em uso, yup exporta tudo como uma exportação nomeada

```js
import * as yup from 'yup'; // para tudo
// ou
import { string, object } from 'yup'; // para apenas o que você precisa
```
...
// você pode tentar e criar um cast de objetos para schemas definidos
schema.cast({
  name: 'jimmy',
  age: '24',
...
> Se você está procurando por uma DSL facilmente serializável para o schema yup , experimente o [yup-ast](https://github.com/WASD-Team/yup-ast)

### Usando um dicionário de locale personalizado

Permite você customizar mensagens padrões usadas pelo Yup, quando nenhuma mensagem é fornecida em um teste de validação.
Se alguma mensagem está faltando no dicionário personalizado, a mensagem de erro vai ser uma padrão do Yup.
...
// agora use schemas Yup depois de você definir seu dicionário personalizado
let schema = yup.object().shape({
  name: yup.string(),
...
Se você preisa de um suporte multilingual, Yup te cobriu. A função `setLocale` aceita funções que podem ser usadas para gerar objetos de erro com chaves e valores de tradução. Apenas pegue esse exemplo e alimente isso na sua biblioteca i18n favorita.

```js
import { setLocale } from 'yup';

setLocale({
  // use chaves de tradução constantes para mensagens sem valores
  mixed: {
    default: 'field_invalid',
  },
  // use funções para gerar um objeto de erro que inclui o valor de um schema
  number: {
