...
export type TestOptionsMessage<
  ...
  R = any
> = string | ((params: Extra & Partial<TestMessageParams>) => R);
...
export interface MixedSchemaConstructor {
  ...
  // tslint:disable-next-line:no-unnecessary-generics
  new <T = {} | null | undefined>(options?: {
    type?: string;
    [key: string]: any;
  }): MixedSchema<T>;
}

export interface MixedSchema<T extends any = {} | null | undefined>
  extends Schema<T> {
  ...
  ): MixedSchema<Exclude<T, undefined | null>>;
...
> extends Schema<T> {
  ...
  required(
    message?: TestOptionsMessage,
  ): NumberSchema<Exclude<T, undefined | null>>;
...
> extends Schema<T> {
  ...
  required(
    message?: TestOptionsMessage,
  ): BooleanSchema<Exclude<T, undefined | null>>;
...
export interface DateSchema<
  T extends Date | null | undefined = Date | undefined
> extends Schema<T> {
  ...
  required(
    message?: TestOptionsMessage,
  ): DateSchema<Exclude<T, undefined | null>>;
...
export interface NotRequiredNullableArraySchema<T>
  extends BasicArraySchema<T, T[] | null | undefined> {
...
export interface NullableArraySchema<T>
  extends BasicArraySchema<T, T[] | null> {
...
export interface NotRequiredArraySchema<T>
  extends BasicArraySchema<T, T[] | undefined> {
...
export interface ObjectSchemaConstructor {
  <T extends object>(fields?: ObjectSchemaDefinition<T>): ObjectSchema<
    T | undefined
  >;
  new (): ObjectSchema<{}>;
}
...
> extends Schema<T> {
  ...
  required(
    message?: TestOptionsMessage,
  ): ObjectSchema<Exclude<T, undefined | null>>;
...
export type AssertingTestFunction<T> = (
  this: TestContext,
  value: any,
) => value is T;
...
interface NonExclusiveTestOptions<P extends Record<string, any>>
  extends BaseTestOptions<P> {
  exclusive?: false;
}

interface ExclusiveTestOptions<P extends Record<string, any>>
  extends BaseTestOptions<P> {
  exclusive: true;
  name: string;
}

interface NonExclusiveAssertingTestOptions<U, P extends Record<string, any>>
  extends NonExclusiveTestOptions<P> {
  test: AssertingTestFunction<U>;
}

interface ExclusiveAssertingTestOptions<U, P extends Record<string, any>>
  extends ExclusiveTestOptions<P> {
  test: AssertingTestFunction<U>;
}

export type TestOptions<P extends Record<string, any> = {}> =
  | NonExclusiveTestOptions<P>
  | ExclusiveTestOptions<P>;
...
type InnerInferType<T> =
  | (T extends Array<infer T>
      ? InnerInferTypeArray<T>
      : Id<NotRequiredProps<T> & RequiredProps<T>>)
  | PreserveOptionals<T>;
