...
SchemaType.prototype = {
  ...
  clone: function () {
    return this.constructor.create(transform(this, function (obj, val, key) {
      obj[key] = val.clone ? val.clone() : Array.isArray(val) ? val.slice() : isPlainObject(val) ? babelHelpers._extends({}, val) : val;
    }, {}));
  },
  ...
  _cast: function (_value) {
    ...
    if (value == null && has(this, "_default")) value = this._nullable && value === null ? value : this.default();
  ...
  default: function (def) {
    if (arguments.length === 0) {
      var dflt = has(this, "_default") ? this._default : this._initialDefault;

      return typeof dflt === "function" ? dflt() : clone(dflt);
    }
...
for (var method in aliases) if (has(aliases, method)) aliases[method].forEach(function (alias) {
